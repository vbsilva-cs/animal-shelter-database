-- Funcionamento: 
	-- Para cada bloco, posicione o cursor apos o caractere ';'
	-- Executar cada argumento ou 'statement' individualmente
    -- ou fazer selecao manual e execução dos blocos
    -- A função de cada uma é descrita no título

-- Gerando informações:

-- 1 - Listar todos os pets cadastrados
-- Obs: Pets com cadastros mais recentes são exibidos primeiro
SELECT id_pet as num_cadastro, nome, especie, raca, idade, data_entrada
FROM Pet
ORDER BY data_entrada DESC;

-- 2 - Exibit o total de Pets cadastrados no último mês
SELECT COUNT(id_pet) AS total_cadastrados
FROM Pet
WHERE data_entrada >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH);

-- 3. processos realizados por cada colaborador
...
-- 5 - Histórico de prescrições feitas em consultas de um pet específico
-- Obs: foi escolhido arbitrariamente o pet de ID 3 (número de matrícula)
-- Como alterar? No condicional 'WHERE', na linha 15, escolher ID desejado
...
-- 8 - Itens com estoque abaixo de 5 unidades
SELECT nome as item, tipo as tipo_uso, quantidade
FROM Estoque
WHERE quantidade < 5;

-- 9 - Adotantes com mais de uma adoção
...
-- 10 - Pets disponíveis para adoção
